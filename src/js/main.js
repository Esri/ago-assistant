require([
  ...
  var search = function() {
    ...
    if (jquery("#searchMenu li.active").attr("data-id")) {
      query += " accountid:" + jquery("#searchMenu li.active").attr("data-id");
    }
    ...
    NProgress.start();
    portal.search(query, 100, "numViews", "desc").then(function(results) {
        ...
        if (jsonValid) {
          ...
          if (editButton.attr("data-container") === "Description") {
            portal
              ...
              .then(function(response) {
                if (response.success) {
                  saveButton
                    .children("span")
                    .attr("class", "fa fa-lg fa-check");
                  saveButton.css("color", "green");
                } else {
                  saveButton
                    .children("span")
                    .attr("class", "fa fa-lg fa-times");
                  saveButton.css("color", "red");
                }
              });
            ...
            saveButton
              ...
              .attr("class", "fa fa-lg fa-spinner fa-spin");
            portal
              .updateData(itemInfo.owner, itemInfo.id, ownerFolder, newJson)
              .then(function(response) {
                if (response.success) {
                  saveButton
                    .children("span")
      ...
      portal.itemDescription(id).then(function(description) {
        portal.itemData(id).then(function(data) {
          ...
          var templateData = {
            ...
            id: id,
            description: JSON.stringify(description, undefined, 2),
          ...
          if (
            ...
            description.typeKeywords.indexOf("Service") === -1
          ) {
            templateData.downloadLink =
              portal.portalUrl +
    ...
    jquery(".content").click(function() {
      ...
      portal.itemData(id).then(function(data) {
        ...
        jquery.each(webmapServices, function(service) {
          ...
          if (
            jquery.inArray(rootOfCurrentUrl, currentUniqueRootUrlVals) === -1
          ) {
        ...
        jquery("#originalTextUserInput").bind("input", function() {
          ...
          jquery("#stringMatchesFoundText").text(
            "Found " +
              numFoundStringMatches.toString() +
              " text " +
              matchText +
              " in this web map's service URLs (case sensitive)."
          );
        });
        ...
        jquery("#bulkUpdateWebmapServiceUrlsBtn").click(function(e) {
          ...
          if (origTextUserInput.length > 0) {
            ...
            jquery.each(webmapServices, function(service) {
              var originalUrl = jquery(webmapServices[service]).val();
              if (originalUrl.indexOf(origTextUserInput) > -1) {
                var newCalculatedUrl = originalUrl.replace(
                  origTextUserInput,
                  newTextUserInput
                );
        ...
        jquery("#btnUpdateWebmapServices").click(function() {
          var webmapServices = jquery("[data-original]");
          jquery.each(webmapServices, function(service) {
            var originalUrl = jquery(webmapServices[service]).attr(
              "data-original"
            );
            var newUrl = jquery(webmapServices[service]).val();

            // Find and replace each URL.
            webmapData = webmapData.replace(
              '"' + originalUrl + '"',
              '"' + newUrl + '"'
            );
          ...
          portal
            .updateWebmapData(owner, folder, webmapId, itemData)
            .then(function(response) {
              var html;
              if (response.success) {
                // Set the stored original URL to the new value.
                jquery.each(webmapServices, function(service) {
                  jquery(webmapServices[service]).attr(
                    "data-original",
                    jquery(webmapServices[service]).val()
                  );
                });

                html = mustache.to_html(
                  jquery("#updateSuccessTemplate").html()
                );
                jquery("#btnResetWebmapServices").before(html);
              } else if (
                response.error.code === 400 || response.error.code === 403
              ) {
                jquery("#btnResetWebmapServices").click(); // Reset the displayed URLs to their original values.
                html = mustache.to_html(
                  jquery("#updateErrorTemplate").html(),
                  response
                );
                jquery("#btnResetWebmapServices").before(html);
              }
  ...
  var updateContentUrls = function() {
    ...
    jquery(".content").click(function() {
      ...
      portal.itemDescription(id).then(function(description) {
        ...
        var html = mustache.to_html(
          jquery("#itemContentTemplate").html(),
          description
        );
        ...
        jquery("#btnUpdateContentUrl").click(function() {
          ...
          portal
            .updateUrl(owner, folder, contentId, url)
            .then(function(response) {
              var html;
              if (response.success) {
                // Set the stored original URL to the new value.
  ...
  var viewStats = function() {
    ...
    portal.userProfile(portal.username).then(function(user) {
      ...
      portal.search(searchQuery, 3, "numViews", "desc").then(function(results) {
        jquery.each(results.results, function(result) {
          results.results[result].numViews = results.results[result].numViews
            .toString()
            .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        ...
        jquery("#mostViewedContent").html(
          mustache.to_html(tableTemplate, {
  ...
  var checkServiceName = function(destinationPortal) {
    ...
    nameInput.blur(function() {
      ...
      destinationPortal.self().then(function(self) {
        destinationPortal
          .checkServiceName(self.user.orgId, name, "Feature Service")
          .then(function(available) {
            if (available.available !== true) {
              var nameError = mustache.to_html(
                jquery("#serviceNameErrorTemplate").html(),
                {
                  name: name
                }
              );
        ...
        .then(function(response) {
          ...
          if (response.success === true) {
            ...
            if (description.url.indexOf("id=") > -1) {
              var newUrl =
                destinationPortal.portalUrl +
                description.url.substring(description.url.indexOf("apps/"));
            ...
            html = mustache.to_html(
              jquery("#contentCopyErrorTemplate").html(),
              {
                id: id,
  ...
  var deepCopyFeatureService = function(id, folder) {
    ...
    destinationPortal
      .createService(
        destinationPortal.username,
        folder,
        JSON.stringify(serviceDefinition)
      )
      .then(function(service) {
        ...
        portal.serviceLayers(description.url).then(function(definition) {
          ...
          var reportResult = function(layerId) {
            ...
            if (
              layerJobs[layerId].attempted >= layerJobs[layerId].recordCount
            ) {
          ...
          destinationPortal
            ...
            .then(function(response) {
              if (!("error" in response)) {
                jquery.each(layers, function(i, v) {
                  var layerId = v.id;
                  portal
                    .layerRecordCount(description.url, layerId)
            ...
            .catch(function() {
              // Catch on addToServiceDefinition.
              clone.find("img").remove();
              clone.removeClass("btn-info");
              clone.addClass("btn-danger");
              messages.text("Failed—check console");
  ...
  var makeDroppable = function(id) {
    ...
    var copyItem = function(id, folder) {
      ...
      if (isSupported(type)) {
        ...
        portal.itemDescription(id).then(function(description) {
          ...
          switch (type) {
            case "Feature Service":
              ...
              portal.items[portal.items.length - 1].description.url =
                description.url;

              portal
                .serviceDescription(description.url)
        ...
        var html = mustache.to_html(
          jquery("#contentTypeErrorTemplate").html(),
          {
            id: id,
            type: type
          }
  ...
  var highlightCopyableContent = function() {
    var setMaxWidth = function(el) {
      ...
      var maxWidth = jquery("#itemsArea .in").width()
        ? jquery("#itemsArea .in").width()
        : 400;
  ...
  var listUserItems = function() {
    ...
    portal.userContent(portal.username, "/").then(function(content) {
      ...
      jquery.each(content.folders, function() {
        ...
        var html = mustache.to_html(
          jquery("#folderTemplate").html(),
          folderData
        );
        jquery("#itemsArea").append(html);
        portal.userContent(portal.username, this.id).then(function(content) {
          // Update the folder count.
          jquery("#collapse_" + content.currentFolder.id)
            .parent()
            .find("span.badge")[0].innerHTML =
            content.total;
  ...
  var listUserGroups = function() {
    ...
    portal.userProfile(portal.username).then(function(user) {
      ...
      jquery.each(user.groups, function() {
        ...
        portal.search(query, 100, "title", "asc").then(function(search) {
          // Update the folder count.
          jquery("#collapse_" + group.id).parent().find("span.badge")[
            0
          ].innerHTML =
            search.total;
  ...
  var showDestinationFolders = function() {
    ...
    portal.userContent(portal.username, "/").then(function(content) {
      ...
      html = mustache.to_html(jquery("#dropFolderTemplate").html(), folderData);
  ...
  jquery(document).ready(function() {
    ...
    jquery(".loginButtons > p > button").removeAttr("disabled");
    ...
    if (sessionStorage.esriJSAPIOAuthBackup && sessionStorage.esriIdBackup) {
      ...
      sessionStorage.setItem(
        "esriJSAPIOAuth",
        sessionStorage.getItem("esriJSAPIOAuthBackup")
      );
    }
    ...
    jquery("[data-action='login-agol']").click(function() {
      esriId
        .getCredential(appInfo.portalUrl, {
    ...
    jquery("[data-action='logindestination']").click(function() {
      ...
      esriId
        ...
        .then(function(user) {
          ...
          if (
            !app.portals.destinationPortal ||
            app.portals.destinationPortal.portalUrl !== appInfo.portalUr
          ) {
            app.portals.destinationPortal = new portalSelf.Portal({
              portalUrl: user.server + "/",
              username: user.userId,
    ...
    jquery(document).on("click", "#searchMenu li", function(e) {
      ...
      if (
        selectedAction !== "viewMyContent" && selectedAction !== "viewMyGroups"
      ) {
    ...
    jquery(document).on("click", "#btnCopyService", function(e) {
      var id = jquery(e.currentTarget).attr("data-id");
      var folder = jquery(".clone[data-id='" + id + "']")
        .parent()
        .attr("data-folder");
